...
{
	return "SELECT [Id_person], [last_name], [first_name] FROM [POO_Prosit6].[dbo].[Clients]";
}
System::String^ NS_Comp_Mappage::CLmapTB::Insert(void)
{
	return "INSERT INTO Clients (last_name, first_name) VALUES('" + this->nom + "','" + this->prenom + "');";
}
System::String^ NS_Comp_Mappage::CLmapTB::Delete(void)
{
	return "DELETE FROM Clients WHERE Id_person = " + this->Id + ";";
}
